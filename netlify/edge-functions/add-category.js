...
const validateRequestData = (requestData) => {
  ...
  if (!name || !image || !api_key || !user_id) {
    throw new Error('Missing required fields: name, image, api_key, and user_id');
  }
...
const insertCategory = async (turso, name, image, createdAt, editedAt, userId) => {
  const query = `
  ...
  if (response.error) {
    console.error(`[ERROR] Failed to insert category: ${response.error.message}`);
  ...
  const newValues = JSON.stringify({ name, image, createdAt, editedAt, is_active: true });
  ...
  if (auditResponse.error) {
    console.error(`[ERROR] Failed to insert audit record: ${auditResponse.error.message}`);
...
const getCategoryById = async (turso, categoryId) => {
  ...
  if (response.error) {
    console.error(`[ERROR] Failed to fetch category: ${response.error.message}`);
...
export default async (request) => {
  ...
  try {
    ...
    console.log(`[SUCCESS] Category inserted successfully. ID: ${newCategoryId}`);
